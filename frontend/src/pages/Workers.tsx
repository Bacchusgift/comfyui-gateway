...
export default function Workers() {
  ...
  const showError = (msg: string) => {
    setErr(msg);
    setTimeout(() => setErr(null), 5000); // 5秒后自动关闭
  };

  const load = () => api.list().then((r) => setList(r.workers)).catch((e) => showError(e.message));
  ...
  const handleDelete = (id: string) => {
    if (!confirm("确定删除该 Worker？")) return;
    api.delete(id).then(load).catch((e) => showError(e.message));
  };

  return (
    <>
      {err && (
        <div className="fixed top-4 right-4 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg shadow-lg z-50 flex items-center gap-3 animate-fade-in">
