...
export default function Workers() {
  ...
  const load = () => api.list().then((r) => setList(r.workers)).catch((e) => setErr(e.message));
  ...
  const handleDelete = (id: string) => {
    ...
    api.delete(id).then(load).catch((e) => setErr(e.message));
  };

  if (err) return <div className="text-red-600">错误: {err}</div>;
